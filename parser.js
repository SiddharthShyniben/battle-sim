    ...
    switch (args[0]) {
      ...
      case "cant": {
        ...
        if (
          ...
          args[0] = "-block";
          return {
            args: ["-block", pokemon, effect, move, kwArgs.of],
            kwArgs: {},
          };
        }
        break;
      ...
      case "-heal": {
        ...
        if (["dryskin", "eartheater", "voltabsorb", "waterabsorb"].includes(id))
          kwArgs.of = "";
        break;
      }
  ...
  template(type, ...namespaces) {
    for (const namespace of namespaces) {
      ...
      if (BattleText[id] && type in BattleText[id]) {
        if (BattleText[id][type].charAt(1) === ".")
          type = BattleText[id][type].slice(2);
        if (BattleText[id][type].charAt(0) === "#")
          id = BattleText[id][type].slice(1);
    ...
    switch (cmd) {
      ...
      case "-ability": {
        ...
        if (id === "unnerve") {
          ...
          return (
            line1 +
            template.replace("[TEAM]", this.team(pokemon.slice(0, 2), true))
          );
        }
        let templateId = "start";
      ...
      case "-item": {
        ...
        const line1 = this.maybeAbility(kwArgs.from, kwArgs.of || pokemon);
        if (
          ["thief", "covet", "bestow", "magician", "pickpocket"].includes(id)
        ) {
      ...
      case "-enditem": {
        ...
        if (!template)
          template = this.template("activateItem").replace(
            "[ITEM]",
            this.effect(item),
          );
        return (
      ...
      case "-singlemove": {
        ...
        if (!template)
          template = this.template("start").replace(
            "[EFFECT]",
            this.effect(effect),
          );
        return (
      ...
      case "-sidestart": {
        ...
        let template = this.template("start", effect, "NODEFAULT");
        if (!template)
          template = this.template("startTeamEffect").replace(
            "[EFFECT]",
            this.effect(effect),
          );
        return template
      ...
      case "-sideend": {
        ...
        if (!template)
          template = this.template("endTeamEffect").replace(
            "[EFFECT]",
            this.effect(effect),
          );
        return template
      ...
      case "-weather": {
        ...
        if (!template)
          template = this.template("startFieldEffect").replace(
            "[EFFECT]",
            this.effect(weather),
          );
        return line1 + template;
      ...
      case "-fieldactivate": {
        ...
        if (BattleTextParser.effectId(effect) === "perishsong")
          templateId = "start";
      ...
      case "-fieldend": {
        ...
        let template = this.template("end", effect, "NODEFAULT");
        if (!template)
          template = this.template("endFieldEffect").replace(
            "[EFFECT]",
            this.effect(effect),
          );
        return template;
      ...
      case "-activate": {
        ...
        if (id === "celebrate") {
          return this.template("activate", "celebrate").replace(
            "[TRAINER]",
            this.trainer(pokemon.slice(0, 2)),
          );
        }
        if (
          !target &&
        ...
        if ((id === "mummy" || id === "lingeringaroma") && kwArgs.ability) {
          ...
          line1 += this.ability(
            id === "mummy" ? "Mummy" : "Lingering Aroma",
            target,
          );
        ...
        if (id === "commander") {
          ...
          return (
            ...
            template
              .replace("[POKEMON]", this.pokemon(pokemon))
              .replace(/\[TARGET\]/g, this.pokemon(target))
          );
        }
        ...
        if (
          (id === "protosynthesis" || id === "quarkdrive") &&
          kwArgs.fromitem
        ) {
      ...
      case "-unboost": {
        ...
        return (
          line1 +
          template
            .replace(/\[POKEMON\]/g, this.pokemon(pokemon))
            .replace("[STAT]", BattleTextParser.stat(stat))
        );
      }
      ...
      case "-immune": {
        ...
        if (!template) {
          ...
          template = this.template(
            pokemon ? templateId : "immuneNoPokemon",
            kwArgs.from,
          );
        }
        return line1 + template.replace("[POKEMON]", this.pokemon(pokemon));
